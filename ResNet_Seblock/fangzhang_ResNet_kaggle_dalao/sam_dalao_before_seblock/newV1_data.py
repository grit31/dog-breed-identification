...
def main():
    ...
    parser.add_argument('--augment', type=bool,default = True, help='是否启用数据增强（不加参数则为无增强）')
    ...
    parser.add_argument('--model_path', type=str, default='resnet18se_adam_aug_80.pth')
    ...
    if args.augment:
        ...
        #     transforms.Resize((256, 256)),               # 先resize到大一点
        #     transforms.RandomResizedCrop(224, scale=(0.8, 1.0)),  # 随机裁剪，仿AlexNet/ImageNet预处理
        #     transforms.RandomHorizontalFlip(p=0.5),
        #     transforms.RandomVerticalFlip(p=0.3),
        #     transforms.RandomRotation(degrees=20),
        #     transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
        #     transforms.RandomAffine(
        #         degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=10, fill=0),  # 仿照真实抖动
        #     transforms.ToTensor(),
        #     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        # ])

        # adam建议只用这些增强，去掉vertical flip/affine
        train_transform = transforms.Compose([
