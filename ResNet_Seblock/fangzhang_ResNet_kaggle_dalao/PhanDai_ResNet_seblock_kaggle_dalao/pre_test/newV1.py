...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default=r'D:\homework\Course_Design_of_artificial_intelligence\third\ResNet_Seblock\fangzhang_ResNet_kaggle_dalao\PhanDai_ResNet_seblock_kaggle_dalao\pre_test\newV1_pre')
    ...
    parser.add_argument('--augment', type=bool, default=True, help='是否启用数据增强')
    ...
    parser.add_argument('--model_path', type=str, default='resnet18se_transfer.pth')
    ...
    if args.optimizer == 'adam' and args.lr == 3e-4:
        args.lr = 3e-4
    elif args.optimizer == 'sgd' and args.lr == 1e-1:
        args.lr = 0.1
    ...
    # ====== 3. 模型初始化（迁移学习/加载预训练）======
    import torchvision
    model = build_resnet18_se(num_classes=1000, reduction=args.reduction).to(args.device)
    ...
    # 替换分类头为任务类别数
    model.fc = nn.Linear(512, len(encoder.classes_)).to(args.device)
