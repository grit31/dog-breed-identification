...
def get_memory_usage(device):
    if device.type == "cuda":
        return torch.cuda.max_memory_allocated(device=device) / 1024 ** 2  # MB
    else:
        import psutil
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default=r'D:\homework\Course_Design_of_artificial_intelligence\third\ResNet\kaggle_dalao\sam_dalao_before\newV1')
    ...
    parser.add_argument('--model_path', type=str, default='resnet18_adam_30.pth')
    ...
    if args.optimizer == 'adam' and args.lr == 0.001:
        args.lr = 0.001
    ...
    # ====== 3. 模型、优化器、损失、调度器 =======
    model = build_resnet18(num_classes=len(encoder.classes_)).to(args.device)
