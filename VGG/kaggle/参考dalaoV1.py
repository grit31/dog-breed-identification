...
# --- 模型构建 ---
model = get_vgg11(num_classes=len(classes))
...
for epoch in range(1, num_epochs+1):
    print(f"==== Epoch {epoch}/{num_epochs} (Device: {device}) ====")
    model.train()
    train_loss = 0.0
    loop = tqdm(enumerate(train_loader), total=len(train_loader), desc='Train', ncols=100)
    ...
    model.eval()
    val_loss = 0.0
    val_loop = tqdm(enumerate(valid_loader), total=len(valid_loader), desc='Valid', ncols=100)
    with torch.no_grad():
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(save_dir, f'loss_curve_epoch{epoch}.png'))
    plt.close()

# --- 推理与输出 ---
model.eval()
model.load_state_dict(torch.load(os.path.join(save_dir, 'vgg11_best.pth'), map_location=device))
results = {}
with torch.no_grad():
...
output_df.columns = [inv_map[i] for i in range(len(output_df.columns))]
output_df = output_df.reset_index().rename(columns={'index': 'id'})
output_df.to_csv(os.path.join(save_dir, 'submission.csv'), index=False)
print('Test predictions saved to', os.path.join(save_dir, 'submission.csv'))
