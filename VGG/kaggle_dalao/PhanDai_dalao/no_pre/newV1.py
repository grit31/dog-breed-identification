...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    top5 = top_k_accuracy_score(targets_all, np.vstack([probs.cpu().numpy() for probs in [torch.softmax(model(imgs.to(device)), dim=1) for imgs, _ in dataloader]]), k=5)
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='dog-breed-identification')
    ...
    parser.add_argument('--model_path', type=str, default='vgg11_best.pth')
